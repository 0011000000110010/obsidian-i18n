...
![[assets/Pasted image 20240924160506.png]]
...
操作指南：
- 在插件设置中启用 `云端文件模式`![[assets/Pasted image 20240924161923.png]]
- 点击侧边栏的i18n按钮 ![[assets/Pasted image 20240924161836.png]]
- 选择对应插件右侧的 `下载` 按钮（`下载` 被点击后，会变成 `翻译` ）![[assets/Pasted image 20240924162000.png]]
- 点击 `翻译` 按钮
- 正常情况下，翻译已生效
...
- 点击侧边栏的i18n按钮
- 点击对应插件右侧的 `提取` 按钮
- 点击新出现的钢笔图案，即 `编辑` 按钮![[assets/Pasted image 20240924164046.png]]
- 点击后 `编辑` 后会弹出内置编辑器，此编辑器可格式化显示json词典 ![[assets/Pasted image 20240924170122.png]]
...
目前支持百度翻译api与兼容OpenAI格式的大语言模型api。对应接口api的获取方式请自行查询资料。
