...
export class AdminView extends ItemView {
    ...
    async onload() {
        ...
        if (this.i18n.issuesObj.title.includes("[提交译文] ")) {
            ...
            originalManifestEl.createEl('input', { value: this.i18n.issuesObj.title.replace("[提交译文] ", ""), cls: 'i18n-edit__plugin-version-input' }).disabled = true;
            originalManifestEl.createEl('span', { text: '旧描述', cls: 'i18n-edit__label-wrap' });
            originalManifestEl.createEl('input', { value: this.issueTranslationJson.description.original, cls: 'i18n-edit__description-input' }).disabled = true;

            translationManifestEl.createEl('span', { text: '插件版本', cls: 'i18n-edit__label-wrap' });
            translationManifestEl.createEl('input', { value: this.issueTranslationJson.manifest.pluginVersion, cls: 'i18n-edit__plugin-version-input' }).disabled = true;
            translationManifestEl.createEl('span', { text: '新描述', cls: 'i18n-edit__label-wrap' });
            translationManifestEl.createEl('input', { value: this.issueTranslationJson.description.original, cls: 'i18n-edit__description-input' }, (el) => {
            ...
            if (this.issueTranslationJson != undefined) {
                ...
                for (const key in this.issueTranslationJson.dict) {
                    if (this.issueTranslationJson.dict.hasOwnProperty(key)) {
                        ...
                        labelEl.createEl('span', { text: '新增', cls: ['i18n-tag'] })
                        ...
                        operateEl.createEl('button', { text: dictItem.state ? '否决' : '批准', cls: ['i18n-edit__operate-operate-button'] }, async (el) => {
                ...
                // [功能] 测试
                new ButtonComponent(searchEl).setClass('i18n-button').setClass('i18n-button--success').setClass('i18n-edit__save-button').setClass('i18n-edit__operate-button').setButtonText('测试').onClick(async () => {
                    // @ts-ignore
                ...
                // [功能] 关闭
                new ButtonComponent(searchEl).setClass('i18n-button').setClass('i18n-button--success').setClass('i18n-edit__save-button').setClass('i18n-edit__operate-button').setButtonText('关闭').onClick(async () => {
                ...
                // [功能] 合并
                new ButtonComponent(searchEl).setClass('i18n-button').setClass('i18n-button--success').setClass('i18n-edit__save-button').setClass('i18n-edit__operate-button').setButtonText('合并').onClick(async () => {
                    try {
                        // 更新翻译版本和字典  
                        this.issueTranslationJson.dict = Object.fromEntries(this.submitTranslationObj.filter(item => item.state === true).map(item => [item.key, item.value]));
        ...
        if (this.i18n.issuesObj.title.includes("[更新译文] ")) {
            ...
            cEl.createEl('input', { value: this.i18n.issuesObj.user.login, cls: 'i18n-edit__plugin-version-input' }).disabled = true;
            cEl.createEl('span', { text: '源代码', cls: 'i18n-edit__label-wrap' });
            cEl.createEl('input', { value: this.issueTranslationJson.description.original, cls: 'i18n-edit__description-input' }).disabled = true;

            originalManifestEl.createEl('span', { text: '插件标识', cls: 'i18n-edit__label-wrap' });
            originalManifestEl.createEl('input', { value: id, cls: 'i18n-edit__plugin-version-input' }).disabled = true;
            originalManifestEl.createEl('span', { text: '旧描述', cls: 'i18n-edit__label-wrap' });
            originalManifestEl.createEl('input', { value: this.cloudTranslationJson.description.translation, cls: 'i18n-edit__description-input' }).disabled = true;

            translationManifestEl.createEl('span', { text: '插件版本', cls: 'i18n-edit__label-wrap' });
            translationManifestEl.createEl('input', { value: this.issueTranslationJson.manifest.pluginVersion, cls: 'i18n-edit__plugin-version-input' }).disabled = true;
            translationManifestEl.createEl('span', { text: '新描述', cls: 'i18n-edit__label-wrap' });
            translationManifestEl.createEl('input', { value: this.issueTranslationJson.description.translation, cls: 'i18n-edit__description-input' }, (el) => {
                el.addEventListener("input", () => { this.issueTranslationJson.description.translation = el.value })
            });
            ...
            if (this.issueTranslationJson != undefined) {
                ...
                for (const key in compare.modified) {
                    ...
                    const diff = this.diff(dictItem.key, dictItem.value);
                    const labelEl = rowEl.createEl('td', { cls: ['i18n-review__cell'] });
                    labelEl.createEl('span', { text: '修改', cls: ['i18n-tag'] })
                    const keyCellEl = rowEl.createEl('td', { cls: ['i18n-review__cell'] });
                    keyCellEl.innerHTML = diff.s1;
                    ...
                    operateCellEl.createEl('button', { text: dictItem.state ? '否决' : '批准', cls: ['i18n-edit__operate-operate-button'] }, async (el) => {
                        el.addEventListener('click', async () => {
                            dictItem.state = !dictItem.state;
                            el.textContent = dictItem.state ? '否决' : '批准';
                        })
                ...
                for (const key in compare.removed) {
                    ...
                    this.updateTranslationObj.push(dictItem);
                    const labelEl = rowEl.createEl('td', { cls: ['i18n-review__cell'] });
                    labelEl.createEl('span', { text: '删除', cls: ['i18n-tag'] })
                    const keyCellEl = rowEl.createEl('td', { cls: ['i18n-review__cell'] });
                ...
                for (const key in compare.unchanged) {
                    ...
                    this.updateTranslationObj.push(dictItem);
                    const labelEl = rowEl.createEl('td', { cls: ['i18n-review__cell'] });
                    labelEl.createEl('span', { text: '初始', cls: ['i18n-tag'] })
                    const keyCellEl = rowEl.createEl('td', { cls: ['i18n-review__cell'] });
                    const valueCellEl = rowEl.createEl('td', { cls: ['i18n-review__cell'] });
                    const operateEl = rowEl.createEl('td', { cls: ['i18n-review__cell'] });
                    operateEl.createEl('button', { text: dictItem.state ? '否决' : '批准', cls: ['i18n-edit__operate-operate-button'] }, async (el) => {
